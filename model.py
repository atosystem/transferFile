...
class TransformerXL(object):
    ...
    def get_model(self, pretrain_model=None):
        ...
        if pretrain_model:
            ...
            st_step = checkpoint['step'] + 1

        else:
            model.apply(self.weights_init)
            model.word_emb.apply(self.weights_init) 
        return st_eopch ,st_step, model.to(self.device)


    def save_checkpoint(self, state, root, save_freq=10):
        if state['epoch'] % save_freq == 0:
            torch.save(state, os.path.join(root,'ep_{}_step_{}.pth.tar'.format(state['epoch'],state['step'])))
    ...
    def train(self, train_data, trainConfig, device, resume,dev_data=None):
        ...
        train_dataloader = DataLoader(train_data, trainConfig['batch_size'], shuffle=True, sampler=None,
           batch_sampler=None, num_workers=trainConfig["dataloader_worker_num"])
        ...
        # tensorboard
        tfb_log_dir = os.path.join(checkpoint_dir,"tfb_log")
        os.makedirs(tfb_log_dir,exist_ok=True)
        print("TensorBoard log dir: {}".format(tfb_log_dir))
        tfb_writer = SummaryWriter(log_dir=tfb_log_dir)
        ...
        for epoch in range(st_epoch, trainConfig['num_epochs']):
            ...
            # for bidx in range(len(train_data)):
            for bidx, sample in enumerate(train_dataloader):
                ...
                # get batch
                n_group = trainConfig["group_size"]
                batch_x = sample["src"][:,:-1].reshape(-1,n_group,trainConfig["seq_len"])
                batch_y = sample["src"][:,1:].reshape(-1,n_group,trainConfig["seq_len"])
                batch_mask = sample["att_msk"][:,:-1].reshape(-1,n_group,trainConfig["seq_len"])
                # print(batch_x.shape)
                # print(batch_y.shape)
                # exit()
                ...
                for gidx in range(n_group):
                    ...
                    if torch.all(group_mask==0):
                        continue
                    
                    group_x = group_x.permute(1, 0).contiguous().to(self.device).long()  # (seq_len, bsz)
                    group_y = group_y.permute(1, 0).contiguous().to(self.device).long()
                    group_mask = group_mask.to(self.device).float()
                    ...
                    sys.stdout.flush()
                tfb_writer.add_scalar(tag="train_loss",scalar_value=sum(train_loss) / len(train_loss),global_step=step)
                tfb_writer.flush()
                optimizer.step()
            ...
            if dev_data is not None:
                self.validate(dev_data=dev_data,trainConfig=trainConfig,device=device,resume=None,model=model,tfb_writer=tfb_writer,trained_step=step)

            if curr_train_loss < 0.01:
                print('Experiment [{}] finished at loss < 0.01.'.format(checkpoint_dir))
                break

    def validate(self, dev_data, trainConfig, device, resume,model=None,tfb_writer=None,trained_step=None):
        checkpoint_dir = trainConfig['experiment_Dir']
        batch_size = trainConfig['batch_size']
        data_ROOT = trainConfig['ROOT']
        ...
        dev_dataloader = DataLoader(dev_data, trainConfig['eval_batch_size'], shuffle=False, sampler=None,
           batch_sampler=None, num_workers=trainConfig["dataloader_worker_num"])
        ...
        # train_y = dev_data['y'] 
        # mask = dev_data['mask'] 
        # num_groups = dev_data['num_groups'] 
        ...
        with torch.no_grad():
            ...
            # for bidx in range(len(dev_data)):
            for bidx, sample in tqdm(enumerate(dev_dataloader)):
                # index
                bidx_st = batch_size * bidx
                bidx_ed = batch_size * (bidx + 1)
            ...
            #val_loss = self.validate(val_data, batch_size, model, trainConfig["seed"], trainConfig['max_eval_steps'])
            curr_eval_loss = sum(eval_loss) / len(eval_loss)
            tfb_writer.add_scalar(tag="eval_loss",scalar_value=curr_eval_loss,global_step=trained_step)
            tfb_writer.flush()
            print("Eval loss :{} | time {}".format(curr_eval_loss,time.time()-st_time))
            return curr_eval_loss




    def inference(self, model_path, token_lim, strategies, params, bpm, output_path):
        _, _, model = self.get_model(model_path)
        model.eval()
        ...
        # add beat
        words[-1].append(self.event2word['SOS'])
        ...
        while len(words[0]) < token_lim:
            ...
            if initial_flag:
                ...
                temp_x = np.zeros((len(words[b][-prompt_len:]), batch_size))
                
                for b in range(batch_size):
                    temp_x[:,b] = words[b][-prompt_len:] ####?####
                # prompt_len = 1
            ...
            if word == self.event2word['EOS']:
                print(" "*os.get_terminal_size().columns,end='\r')
                print("Piece End!!")
                break
            else:
                print(len(words[0]),self.word2event[word])
            # if word == self.event2word['SOS']:
            #     generate_n_bar += 1
            #     print(" "*os.get_terminal_size().columns,end='\r')
            #     print("Generate Bar#{} tokens #{}".format(generate_n_bar,len(words[0])),end='\r')
            # elif word == self.event2word['Piece-End']:
            #     print(" "*os.get_terminal_size().columns,end='\r')
            #     print("Piece End!!")
            #     break
            # else:
            #     # print(" "*os.get_terminal_size().columns,end='\r')
            #     # print("Generate Track #{} Bar#{} :{}".format(generate_n_track,generate_n_bar,word),end='\r')
            #     pass
        
        with open("original_generated.pkl","wb") as f:
            pickle.dump(words[0],f)
        myvocab.toPkl(tokenIDs=words[0],output_pkl_path=output_path)
        # myvocab.toMidi(output_path,tokenIDs=words[0])
