...
class TransformerXL(object):
    ...
    def trainMulti(self, train_data, trainConfig, device, resume):
        return self.train(train_data, trainConfig, device, resume)
    ...
    def train(self, train_data, trainConfig, device, resume):
        ...
        para_model = torch.nn.DataParallel(model, dim=1).to(self.device)
        ...
        for epoch in range(st_epoch, trainConfig['num_epochs']):
            ...
            for bidx, sample in enumerate(train_dataloader):
                ...
                for gidx in range(n_group):
                    ...
                    ret = para_model(group_x, group_y, group_mask, *mems)
    ...
    def inference(self, model_path, token_lim, strategies, params, bpm, output_path):
        ...
        # add beat
        words[-1].append(self.event2word['Piece-Start'])
        ...
        while len(words[0]) < token_lim:
            ...
            if initial_flag:
                ...
                temp_x = np.zeros((prompt_len, batch_size))
                
                for b in range(batch_size):
                    temp_x[0][b] = words[b][-prompt_len] ####?####
                prompt_len = 1
            ...
            # record n_bar
            if word == self.event2word['Bar-Start']:
                generate_n_bar += 1
